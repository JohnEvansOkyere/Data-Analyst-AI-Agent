...
def create_chart(df, chart_type, x_col, y_col=None, color_col=None, title=""):
    ...
    try:
        if chart_type == "Histogram":
            fig = px.histogram(df, x=x_col, color=color_col, title=title, barmode='stack')
        ...
        # Apply dark template for consistency with Streamlit dark mode
        fig.update_layout(
            template='plotly_dark',  # Change to 'plotly_white' if you prefer light mode
            height=500,
            legend=dict(
                bgcolor='rgba(0,0,0,0)',  # Transparent background for legend
                bordercolor='rgba(255,255,255,0.3)',  # Subtle border
                borderwidth=1,
                font=dict(color='white')  # White text for dark backgrounds
            )
        )
        return fig
    ...
    with tab2:
            ...
            for i in range(0, min(6, len(numeric_cols)), 2):
                ...
                with col1:
                    if i < len(numeric_cols):
                        ...
                        fig.update_layout(template='plotly_dark', height=300)  # Consistent theme
                        st.plotly_chart(fig, use_container_width=True)
                with col2:
        ...
        if len(numeric_cols) > 1:
            ...
            fig = px.imshow(corr_matrix,
                            ...
                            aspect='auto')
            fig.update_layout(template='plotly_dark', height=500)  # Consistent theme
            st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        ...
        if len(numeric_cols) >= 2:
            ...
            if len(selected_cols) >= 2:
                ...
                fig.update_layout(template='plotly_dark', height=800)  # Consistent theme
                st.plotly_chart(fig, use_container_width=True)
